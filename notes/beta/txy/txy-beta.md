...
- **Token** — The unit of charge: a metered, spendable unit drawn down as an identity consumes platform resources. The name is the definition — a Token is consumed, Granted, and Refunded, and a balance of Tokens is what an identity holds. (The code now uses `TokenKind`/`TokenBalances` and the `/internal/tokens/*` routes; the one frozen legacy name is the physical table `billing_credit_ledger`, and serialized wire keys keep their `credits`-era names via serde bridges.)
...
- **Entitlement** — A derived view of what an identity may do right now: its access tier (a projection of Plan state) plus its Token balances (from the Ledger). The `Entitlements { plan, tokens }` the webapp reads (the serialized JSON keeps the legacy `credits` key via a serde bridge). Account-layer projection, not an orchestration entity.
