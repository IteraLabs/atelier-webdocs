...
Part of the **State Machine Atlas v0.1-beta-2**. See `fsm-beta.md` for the reading guide, notation, §2.4 FSMs - scope status, and §3 Cross-FSM Sequences. Owns the `INV-SK*` invariant prefix. Cross-references: `task-beta.md` (§2.2), `binding-beta.md` (§2.3), `compute-slot-beta.md` (§2.8), `channel-beta.md` (§2.10). Participates in **SEQ-1 Deploy** (`sequences-beta.md`) - SK-T1/SK-T2 reach Ready synchronously during the first ArtifactChannel CH-T1 against the Sink; INV-SK4 gates B-T3 (Binding Active) on every Manifest-declared Sink being Ready.

This section specifies **only the provisioning states** (Idle -> Ready). Internal states (Streaming, Writing, Backpressured, Error, Closed) are enumerated for cross-reference; their transitions/invariants are deferred work.
...
│     ┌──────────┼──────────┬──────────┐                                       │
│     ▼          ▼          ▼          ▼                                       │
│  Streaming  Writing  Backpressured  Error   (deferred)             │
│     │          │          │          │                                       │
│     └──────────┴──────────┴──────────┤                                       │
│                                      ▼                                       │
│                                    Closed   (deferred)             │
...
Entry: Sink creation (initial write); SK-T3 (deferred release from Ready after Task termination in a later revision).
