...
// `atelier-overseer/atelier-overseer/models`; it does NOT change the wire
...
message Envelope {
    ...
    oneof payload {
        ...
        TerminalEvent         terminal_event          = 32;  // live TerminalSink feed (data.proto); best-effort, non-lineage; backs the webapp DATA-LOGS viewer
...
// CATALOG↔CODE DIVERGENCE (txy-fsm-code #2/#21): live atelier-proto types
// `Registration.skills` and `AgentRegistered.skills` as `repeated Skill` (enum),
// not `repeated string`. The code is the stronger-typed side; the agreed
// resolution is to adopt the enum here (the string→enum wire flip is tracked as
// action #21). Until then treat `repeated Skill` as authoritative on the wire.
...
// during Phase A (SEQ-1 step 4, `sequences-beta.md §3.1.1`); the persisted
// `manifests.body` already carries the canonical `[[metadata.tasks]]`
// block (INV-M3 / INV-M4, `manifest-beta.md §4`). The Gateway's handshake
...
// - preserved for future uses (boot reconciler republish, cross-
// replica coordination, future Gateway federation). See `manifest-beta.md
// §2.4` and §6.
//
// The gRPC `Manifest` / `ManifestAck` messages defined here are reserved
// for the gRPC `ManifestChannel` path; both shapes carry
// equivalent payload - see `manifest-beta.md §7` for the correspondence table.
...
// in a separate enum so the multi-Task (later) `ManifestAck` shape can
// carry per-Task rejection reasons without bloating `ErrorKind` with
// Task-local variants.
//
// Under the v0.1-beta-2 single-Task Manifest restriction
// (taxonomy §Tasks / beta scope; enforced at `binding-beta.md §2.3.2 B-T1`),
...
// the remaining accepted siblings via `binding-beta.md IR-BT2`. The value is
// preserved so that lifting the beta restriction does not require a
// proto schema bump.
//
// See `errors-beta.md` preface for the `ErrorKind` vs `TaskRejectionReason`
// boundary, and `task-beta.md §2.2.2 T-T2`, `binding-beta.md §2.3.2 B-T10` for
...
enum TaskRejectionReason {
    ...
    TASK_REJECTION_PEER_REJECTED    = 4;  // dormant in v0.1-beta-2; reachable in a later version (multi-Task)
}
...
enum CommandKind {
    ...
    TASK_RESUME                = 42;
    TASK_COMPLETE              = 43;
    TASK_CANCEL                = 44;  // dormant: no backing Atlas transition; resolves via T-T8 -> T-T9/T-T10 (see Wire->FSM table below)
}
...
// proto messages per ArtifactKind is deferred.
...
// taxonomy §Activation Modes). The oneof is present so Experiment (later)
...
message ArtifactLineage {
    ...
    oneof activation {
        ...
        string experiment_id = 11;  // reserved
    }
}
...
//     `schema-beta.md §10 artifacts`. `payload_schema_ref` is NOT required in
//     this shape - it would point at out-of-band content the upstream
//     never carries. this path is landed
...
message ArtifactFrame {
    ...
    bytes  payload                       = 10;  // empty in lineage-only mode
...
// originating Manifest row in persistence (`schema-beta.md manifests`).

message ManifestArtifactFrame {
    string artifact_id                   = 1;
    string service_id                    = 2;   // or experiment_id (later version)
...
// TerminalEvent — the live, ephemeral TerminalSink feed surfaced in the webapp
// DATA-LOGS viewer. Best-effort and NOT lineage-bearing (no Artifact ID, no
// `artifacts` row). Rides Envelope.payload tag 32. (txy-fsm-code #2: added to
// the catalog to match the live atelier-proto; webapp is the sole consumer.)
...
|---|---|---|
| AgentRegistered | A-T1 | agent-beta.md §2.1 |
| AgentReady | A-T2 | agent-beta.md §2.1 |
| AgentBound | A-T3 | agent-beta.md §2.1 |
| AgentRestarting | A-T8 (entry) | agent-beta.md §2.1 |
| AgentLost | A-T11 | agent-beta.md §2.1 |
| AgentDraining | A-T5 | agent-beta.md §2.1 |
| AgentTerminated | A-T7 / A-T15 / A-T16 | agent-beta.md §2.1 |
| TaskAccepted | T-T1 | task-beta.md §2.2 |
| TaskRunning | T-T3 | task-beta.md §2.2 |
| TaskPaused / Resumed | T-T4 / T-T5 | task-beta.md §2.2 |
| TaskCompleted | T-T6 / T-T9 | task-beta.md §2.2 |
| TaskFailed | T-T7 / T-T10 | task-beta.md §2.2 |
| TaskCanceled | no FSM transition (retained wire member; TASK_CANCEL resolves via T-T8 -> T-T9 / T-T10) | task-beta.md §2.2 |
| TaskRestarted | T-T13 | agent-beta.md §2.1.5 |
| BindingCreated | B-T1 | binding-beta.md §2.3 |
| BindingActive | B-T3 | binding-beta.md §2.3 |
| BindingDraining | B-T4 | binding-beta.md §2.3 |
| BindingReleasing | B-T5 | binding-beta.md §2.3 |
| BindingReleased | B-T8 | binding-beta.md §2.3 |
| ServiceProvisioning | SV-T1 | service-beta.md §2.5 |
| ServiceDeploying | SV-T2 | service-beta.md §2.5 |
| ServiceActive | SV-T3 | service-beta.md §2.5 |
| ServiceStopping | SV-T6 | service-beta.md §2.5 |
| ServiceStopped | SV-T9 / SV-T10 | service-beta.md §2.5 |
| ServiceArchived | SV-T11 | service-beta.md §2.5 |
| SessionCreated | SN-T1 | session-beta.md §2.7 |
| SessionActive | SN-T2 | session-beta.md §2.7 |
| SessionRenewed | SN-T5 | session-beta.md §2.7 |
| SessionExpiring | SN-T3 | session-beta.md §2.7 |
| SessionExpired | SN-T4 / SN-T6 | session-beta.md §2.7 |
| SessionClosed | SN-T7 | session-beta.md §2.7 |
| ComputeSlotReserved | CS-T1 | compute-slot-beta.md §2.8 |
| ComputeSlotOccupied | CS-T2 | compute-slot-beta.md §2.8 |
| ComputeSlotReleasing | CS-T3 | compute-slot-beta.md §2.8 |
| ComputeSlotVacant | CS-T4 | compute-slot-beta.md §2.8 |
| ComputeSlotRetired | CS-T6 | compute-slot-beta.md §2.8 |
| GatewayReady | GW-T2 | gateway-beta.md §2.9 |
| GatewayDegraded | GW-T3 | gateway-beta.md §2.9 |
| GatewayStopping | GW-T5 | gateway-beta.md §2.9 |
| GatewayStopped | GW-T6 | gateway-beta.md §2.9 |
| ChannelOpening | CH-T1 | channel-beta.md §2.10 |
| ChannelOpen | CH-T2 | channel-beta.md §2.10 |
| SinkIdle | SK-T1 | sink-beta.md §2.11 |
| SinkReady | SK-T2 | sink-beta.md §2.11 |
| OverseerReady | O-T1 | overseer-beta.md §1.3 |
| OverseerDegraded | O-T3 | overseer-beta.md §1.3 |
| OverseerRecovered | O-T4 | overseer-beta.md §1.3 |
| OverseerDraining | O-T6 | overseer-beta.md §1.3 |
...
Closes the multi-tenant leak where `GET /ws/live` would broadcast Service/Binding/Worker events across the whole platform (divergence report §7.2). Required (`/ws/live` consumes `EventSubscribeRequest` rather than a session-less HTTP upgrade).

---

## Design notes

**Opaque `google.protobuf.Struct` for future-typed payloads.** `Command.params`, `TaskSpec.params`, `Registration.host_identity`, and `TaskTelemetry.TaskProgress.metrics` are typed in a later version. `Struct` keeps them on the proto wire while preserving JSON-shape flexibility; lower conversion cost than a bytes-wrapped JSON blob and no separate serde layer on the SDK side.
...
- [fsm-beta.md - preface](fsm-beta.md)
- [overseer-beta.md - §1.3 Overseer states, §1.4 Matrix, §1.6 IR-O4](overseer-beta.md)
...
- [schema-beta.md - persistence schema referenced by wire types](schema-beta.md)
- [../txy/txy-beta.md - Skills, Channels, Sinks taxonomies](../txy/txy-beta.md)
