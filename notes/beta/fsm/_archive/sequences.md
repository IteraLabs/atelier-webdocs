...
SEQ-1 Deploy and SEQ-2 Delete are fully specified in v0.1-beta-2 - together they close the Bring-Your-Own-Infra use-case steps 1-6 (`../txy-v0.1-beta-2/use-case.md`). SEQ-4 Overseer Crash Recovery and SEQ-5 Graceful Shutdown are specified as of **Iteration 5** (§3.4, §3.5 below) - together they close use-case step 7 (Platform Drain -> researcher-side resilience -> Recovery). Their prerequisites - the `channels` persistence table (`schema.md` table 12) and the `agent.reconnect_grace_ms` tunable (`timeouts.md §T.2`) - landed in the same iteration. SEQ-3 Agent Disconnect (enumerated in `fsm-main.md §3`) remains a thin sequence: it is the §3.2.3 "Agent Lost" failure branch generalized, and is folded into SEQ-4 Stage 2 reconciliation rather than given its own section.
...
| Gateway | `gateway.md §2.9` |
| CommandChannel, ManifestChannel, TelemetryChannel | `channel.md §2.10` |
| Overseer | `overseer.md §1` |
| Session | `session.md §2.7` |
| Agent (RemoteAgent) | `agent.md §2.1` |
| Service | `service.md §2.5` |
| Binding | `binding.md §2.3` |
| Workspace | row only (`schema.md`) |
| Pipeline | row only (`schema.md`) |
| ComputeSlot | `compute-slot.md §2.8` |
| Task | `task.md §2.2` |
| Sink | `sink.md §2.11` |
| ArtifactChannel | `channel.md §2.10` |
| Persistence | `schema.md` (Postgres) |
...
The pause window between Phase A return and Phase B start is governed by `binding.pending_grace_ms` (`timeouts.md §T.5`, default 60s). If Phase B does not begin within that window, the live grace ticker (`overseer.md §1.7`) fires B-T2 with `release_reason = 'registration_grace_timeout'` and the Binding is released without ever activating. See §3.1.3 for the failure branch and §3.1.6 for recovery semantics around the pause point.
...
1. **Operator submits Service spec.** Webapp `POST /api/services` with `manifest_toml` body. Overseer admits via `check_availability(SystemOperation::Deploy)` per §1.4 (`overseer.md`).
...
9. **Pause point.** Overseer-side state at end of Phase A:
   ...
   The grace timer (`binding.pending_grace_ms`) is now armed. The live ticker (`overseer.md §1.7`) sweeps Pending Bindings older than the grace window and fires B-T2 on expiry. The boot reconciler (`§1.7` Step 1) covers the same B-T2 cascade for the Overseer-restart case.
...
12. **Gateway forwards Registration on Kafka `lifecycle.events`.** Event arrives at Overseer's `gateway_consumer` (Wave 3.2 / 3.3 wiring). The consumer fires **A-T1** (creation -> Registered).
...
22. **B-T3 cascades drive A-T3 and SV-T3 (atomic with respect to step 21's transaction).**
    - **A-T3** (Ready -> Bound) per IR-BA2. `UPDATE agents.status='bound'`. Emit: Event `AgentBound` (typed `wire::Event::AgentBound` via `gateway_consumer` per Wave 3.2 decision 3.2-7).
    - **SV-T3** (Deploying -> Active). `UPDATE services.status='active'`, `activated_at=now()`. Emit: Event `ServiceActive`.

23. **Overseer fires T-T1** (creation -> pending) for each Task in the Manifest.
...
25. **Overseer fires T-T2** (pending -> accepted).
...
Webapp        Researcher        Gateway          Overseer         Persistence        Event bus
  ...
  |               |                 |                 |--T-T1----------->|                 |
  |               |                 |                 |  [INS tasks]     |                 |
  |               |---Cmd{TaskStart>|---------------->|--T-T2----------->|-TaskAccepted--->|
  |               |                 |                 |  [UPD tasks]     |                 |
  |               |                 |                 |--CS-T2---------->|-CSOccupied----->|
  |               |                 |                 |  [UPD slots]     |                 |
...
- **Deploy rejected (`check_availability` guard at step 1)** - Overseer not in `ready` per §1.4. Error: `OVERSEER_DRAINING` (72) / `SUBSYSTEM_LOSS_*` (70+) per the §1.4 cell. No rows land. Webapp shows the error directly.
...
- **Registration grace timeout (B-T2 grace path)** - `binding.pending_grace_ms` (default 60s) elapses with `bindings.status = 'pending'` and no Registration arriving on `lifecycle.events`. Live grace ticker (`overseer.md §1.7`) fires **B-T2** with `release_reason = 'registration_grace_timeout'`. ComputeSlot reservation released via CS-T3 (Reserved -> Vacant). Service rolls forward via SV-T10 (last-Binding-released auto-stop, see `service.md §2.5`) with `stopped_reason = 'delete_last_agent'` - beta reuses this reason because the slot was never occupied. Webapp polls `GET /api/services/{id}` and sees the Stopped state. Researcher can re-deploy.
...
- **Skill mismatch (A-T2 guard at step 14)** - Agent's declared Skills do not cover the Manifest's Task Skills. Error: `SKILL_MISMATCH` (31). Agent stays in `registered`; eventually reconciled by `agent.registration_window_ms` (A-T12 -> Terminated, `registration_expired`); Binding stays in `pending` until B-T2 grace fires.
...
- **Agent heartbeat miss mid-Phase-B (A-T5 at any step ≥ 15)** - Three missed heartbeats (`agent.heartbeat_miss_threshold_ms` = 15s). Binding cascades to Draining (B-T4); Service cascades. The `§2.1.5` restart lineage may apply - see SEQ-3 post-beta.
...
|---|---|---|
| INV-O4 (envelope atomicity - via IR-O4) | 2, 12 | overseer.md §1.6 |
| INV-SN1 (Session envelope atomicity, row-lock) | 2, 12 | session.md §2.7 |
| INV-SV2 (Service refs Session in active/expiring) | 2 | service.md §2.5 |
| INV-SV1 (Service-Binding count consistency) | 6, 22 | service.md §2.5 |
| INV-A10 (Agent has non-empty Skills before Ready) | 14 | agent.md §2.1 |
| INV-B1 (Binding has Active only after Channels Open) | 21 (B-T3 guard) | binding.md §2.3 |
| INV-B2 (Binding's manifest_id immutable) | 6, 21 | binding.md §2.3 |
| INV-B5 (release_reason immutable) | branch §3.1.3 B-T2 grace path | binding.md §2.3 |
| INV-T1 (Task has compute_slot_id from Accepted onward) | 25 | task.md §2.2 |
| INV-CS1 (1:1 Task <-> Slot runtime) | 26 | compute-slot.md §2.8 |
| INV-CS3 (Pipeline single-activation, v0.1 scope) | 5 (partial index) | compute-slot.md §2.8 |
| INV-CS7 (field-fill consistency) | 5, 26 (CHECK constraint) | compute-slot.md §2.8 |
| INV-CH1 (Open implies handshake complete) | 10, 15, 27, 28 (CH-T2) | channel.md §2.10 |
| INV-CH2 (Channel ID uniqueness) | 10, 15, 27 (CH-T1) | channel.md §2.10 |
| INV-CH6 (ManifestChannel embedded) | 17 | channel.md §2.10 |
| INV-SK1..INV-SK4 (Sink provisioning) | 18, 19 | sink.md §2.11 |
| INV-P1 (Envelope wraps every cross-boundary message) | every wire step | proto-catalog.md |
| INV-P2 (`session_id` required post-registration) | every post-step-13 envelope | proto-catalog.md |
| INV-P6 (Event delivery is Session-scoped) | every Event emission | proto-catalog.md |
| INV-M3 (`accept_task` (T-T1) commits before `manifests.body` is persisted) | 4 (composition + augmentation), 6 (B-T1 commit); precedes step 23 spec ordering | manifest.md |
| INV-M4 (`manifests.body` carries the FULL augmented shape, never rewritten) | 6 (B-T1 commit), 17 (delivered to agent verbatim - via gRPC handshake under BYO-infra) | manifest.md |

### §3.1.5 Timing budget

Reference values live in `timeouts.md`; this table is informational. Phase A is bounded by Overseer transaction budgets (sub-second on a healthy DB); Phase B is bounded by Agent-side handshakes plus the pause window.
...
- **Crash inside Phase B.** Stage 1 re-probes CommandChannel, TelemetryChannel, ArtifactChannel for connected Agents. Channels stay Open through Overseer restart (INV-GW1; the Gateway is a separate process from the Overseer and survives the restart). Stage 2 sweeps `agents` / `bindings` / `tasks` rows. A Binding in `pending` whose Agent has reached `ready` is resumed at step 17 (Manifest transmit) - idempotent via `manifests.idempotency_key`. A Binding in `pending` whose Agent is in `lost` cascades B-T7 + B-T6 with `release_reason='agent_lost'` (post-beta when SEQ-3 lands; v0.1-beta-2 conservatively cascades B-T2 grace).

- **BYO-infra delivery: Phase A atomicity simplifies the crash window (v0.1-beta-2).** Under handshake-delivery the agent never waits on a server-internal transport - the augmented `manifests.body` is either committed (the gRPC handshake will return it on reconnect) or not (Phase A's transaction stack rolled back; the boot reconciler's existing "stale pending Binding" sweep handles it). No agent-side timeout, no `manifests.transmitted_at IS NULL` republish branch. The server-internal Kafka publish on `control.manifests` may have failed during Phase A - that publish is not load-bearing for v0.1-beta-2 (`manifest.md §2.4` / §5.4), so the failure is logged but does not affect agent delivery. Iteration 5+ work that adds a `control.manifests` consumer will revisit this trade-off.
...
| Gateway | `gateway.md §2.9` (unchanged from SEQ-1; INV-GW1 preserves CommandChannel through the sequence) |
| CommandChannel, TelemetryChannel, ArtifactChannel(s) | `channel.md §2.10` (provisioning-contract only; close is transport-layer, not a normative FSM transition in v0.1) |
| Overseer | `overseer.md §1` |
| Session | `session.md §2.7` |
| Agent (RemoteAgent) | `agent.md §2.1` (A-T4 Bound -> Ready; new A-T16 Ready -> Terminated) |
| Service | `service.md §2.5` (SV-T10 Active -> Stopped shortcut) |
| Binding | `binding.md §2.3` (B-T4 -> B-T6 -> B-T8) |
| Workspace | row only (`schema.md workspaces.destroyed_at`) |
| ComputeSlot | `compute-slot.md §2.8` (CS-T3 -> CS-T4) |
| Task | `task.md §2.2` (T-T8 Running -> Stopping -> T-T9 Completed) |
| Sink | `sink.md §2.11` (provisioning-contract only; teardown is operational) |
| Artifacts | `schema.md artifacts` (final DataArtifact lineage row from the drain flush) |
| Persistence | `schema.md` (Postgres) |
...
The pause window between Phase A return and CommandAck arrival is bounded by `gateway.command_ack_window_ms` (5s; see §3.2.5). On expiry, `Scheduler::sweep_pending_commands` (`overseer.md §1.7` Step 2 + the live ticker per §1.8.2) routes through `handle_agent_lost` per §3.2.3 Agent Lost branch.
...
6. **IR-BT3 cascades: T-T8** fires (Running -> Stopping) for every non-terminal Task on the Binding. Under the v0.1 single-Task restriction this is exactly one Task.
   ...
   - Start `stop_drain_timeout` timer per Task (default 60s, `timeouts.md task.stop_drain_timeout_ms`).
7. **Agent (SDK) drains.** Flushes in-flight buffers, completes the in-progress Emit. Produces a final Artifact per open ArtifactChannel:
   ...
   - Persist lineage rows on the platform side as frames land: `INSERT INTO artifacts (artifact_id, session_id, service_id, task_id, kind, sink_id, restart_epoch, sequence, payload_schema_ref, partial, emitted_at, ...) VALUES (...)`. `artifacts` is table 10 per `schema.md`.
...
13. **Overseer destroys the Workspace** atomically with step 12's B-T6 transaction. Persist: `UPDATE workspaces SET destroyed_at=now() WHERE workspace_id=<BND-*.workspace_id>` issued in the same transaction as the B-T6 `UPDATE bindings`. No FSM for Workspace in v0.1; workspace is service-lifecycle-scoped and cleared as an effect of the Binding release. Implementation note (Iteration 4.4): the workspace destroy effect lives inside the `transition_binding_to_releasing` primitive (B-T6) and the `force_release_binding` primitive (B-T7, agent-lost path per §3.2.3); the SV-T10 primitive at step 16 no longer carries this effect (Iteration 3.3 code's SV-T10-owned destroy is superseded).
...
|---|---|---|
| INV-B4 (Binding monotone progression) | 5, 12, 14 | `binding.md §2.3.3` |
| INV-B5 (`release_reason` immutable) | 12 (entry to Releasing) | `binding.md §2.3.3` |
| INV-B7 (Releasing implies Tasks terminal) | 14 (B-T8 guard) | `binding.md §2.3.3` |
| INV-T5 (terminal absorbingness) | 8 (T-T9) | `task.md §2.2.3` |
| INV-A3 (Ready has no active work) | 15 (A-T4), 20 (A-T16 guard) | `agent.md §2.1.3` |
| INV-A5 (Stop is PlatformAgent-only) | 17 - `AGENT_TERMINATE ≠ Stop`; `A-T16` does not violate | `agent.md §2.1.3` |
| INV-A7 (Terminated is absorbing) | 20 | `agent.md §2.1.3` |
| INV-CS1 (1:1 Task <-> Slot runtime) | 11 (CS-T4 post-condition) | `compute-slot.md §2.8` |
| INV-CS7 (field-fill consistency) | 11 (CHECK constraint on `compute_slots`) | `compute-slot.md §2.8` |
| INV-SV6 (auto-stop excludes `task_rejected`) | 16 (SV-T10 guard) | `service.md §2.5` |
| INV-SN1 (Session envelope atomicity, row-lock) | 16, 20 | `session.md §2.7` |
| INV-P1 (Envelope wraps every cross-boundary message) | every wire step | `proto-catalog.md` |
| INV-P3 (one Event variant per boundary-crossing transition) | all emitted Events | `proto-catalog.md` |
| INV-P4 (`restart_epoch` propagates) | 7 (final ArtifactFrame) | `proto-catalog.md` |

### §3.2.5 Timing budget

Reference values live in `timeouts.md`; this table is informational.
...
If the Overseer crashes mid-sequence, `overseer.md §1.7` Recovery applies:
...
- **Channel state during SEQ-2** - even before the `channels` table landed (Iteration 5, `schema.md` table 12), SEQ-2 Recovery was safe because all channels in play are already terminal or terminating when Recovery runs - there is no ongoing steady-state Artifact emission to reconcile. With the durable table now present, SEQ-4 Stage 1 re-probes any residual SEQ-2 channel rows the same way it does for SEQ-5; SEQ-2's correctness does not depend on it.

### §3.2.7 Open issues

- **Resolved 2026-04-23 - `release_reason` enum alignment.** `schema.md bindings.release_reason` CHECK now equals `binding.md §2.3.1` enum: `{normal, operator_stop, force, agent_lost, task_rejected, task_failed}`. Previously-extraneous values (`task_completed`, `drain`, `session_expired`, `delete_agent`) were never written by any FSM transition and have been dropped. SEQ-2 happy path writes `operator_stop`; drain-timeout branch writes `force`; `agent_lost` branch writes `agent_lost`. All three values are now in both the FSM enum and the CHECK.
- **Resolved 2026-04-23 - `tasks.status` enum alignment.** `schema.md tasks.status` CHECK now equals `task.md §2.2.1` state names: `{pending, accepted, running, paused, restarting, stopping, completed, failed, rejected}`. `'pausing'`/`'resuming'`/`'completing'` phase names (atomic in the FSM driver, never persisted) were dropped; `'submitted'`/`'canceled'` were renamed to `'pending'`/`'rejected'` to match FSM state names; `'restarting'` and `'stopping'` were added. Timestamp columns renamed accordingly (`submitted_at -> pending_at`, `canceled_at -> rejected_at`) and `stopping_at` added for drain-duration observability at `T-T8`.
- **`AGENT_TERMINATE` as a System Operation primitive.** A-T16 is new in v0.1-beta-2 (this change). If post-beta work introduces a first-class `Delete` wire message that bundles both sub-Commands, Phase A / Phase G merge; the underlying FSM transitions remain the same.
- **Wire `TASK_CANCEL` / `TaskCanceled` without matching FSM transition.** `CommandKind.TASK_CANCEL = 44` and `Event.TaskCanceled` exist in `proto-catalog.md` but no `T-T` transition writes a `canceled`-equivalent state today. Reconcile post-beta either by removing the enum members or by introducing a `Canceled` state in `task.md §2.2.1`.
- **`services.stopped_reason` broader than `stopped_reason_intent`.** Same class of precedence-alignment drift as the two resolved above; not exercised by Step 6 so left for a follow-up pass.

---

## SEQ-4: Overseer Crash Recovery

The Overseer process restarts (crash, OOM-kill, deploy rollout) and must reconcile durable state before resuming work. The sequence begins at process boot in `initializing` (§1.2) and ends with the Overseer in `ready` (all subsystems healthy, all in-flight entities reconciled) or `degraded` (a subsystem is still down). This is **not** an operator- or Agent-triggered sequence - it is self-driven on boot. It is the cross-FSM trace of the reconciliation protocol specified normatively in `overseer.md §1.7`; this section adds the durable-`channels`-table view and the per-FSM transition accounting that §1.7's prose leaves implicit. Backs `../txy-v0.1-beta-2/use-case.md §7` (the platform half of Drain -> Recovery; the researcher-side half is SEQ-5).
...
| Role | FSM / system |
|---|---|
| Overseer | `overseer.md §1` (T1 Initializing -> Ready / Degraded) |
| Gateway | `gateway.md §2.9` - **separate process, survives the Overseer crash** (IR-O6) |
| Agent(s) (RemoteAgent) | `agent.md §2.1` - survive on client hardware; may be `bound`, `draining`, or `lost` from the Overseer's pre-crash view |
| Session | `session.md §2.7` (rows reloaded; envelope counters re-derived) |
| Service / Binding / Task / ComputeSlot | rows reloaded and swept |
| Channel | `channel.md §2.10` - re-probed via the durable `channels` table (table 12) |
| Command | `schema.md commands` - pending log swept (`overseer.md §1.7` step 2) |
| Persistence | `schema.md` (Postgres) - **assumed recovered**; if down, SEQ-4 cannot start (Overseer stays `initializing`/`degraded`) |
...
Stages mirror `overseer.md §1.7`. Persistence reads/writes are bracketed.
...
11. **Settle Overseer state**: all four subsystems healthy -> O-T1 `OverseerReady` (S-O3 Ready). A subsystem still down -> O-T2 `OverseerDegraded` (S-O4 Degraded), subsystem-loss flag retained, the `overseer.degraded_refresh_ms` (5s) probe loop continues until O-T3 `OverseerRecovered`. **Only now** are System Operations admitted (INV-O5).
...
(boot)        Persistence        Gateway           Channels(tbl)      Event bus
  ...
  |--settle: O-T1 Ready | O-T2 Degraded ---------------------------------->| OverseerReady
  |                                                                          | /Degraded
```
...
|---|---|---|
| INV-O5 (Reconciliation before Ready) | 11 (settle gate) | `overseer.md §1.5` |
| INV-O1 (Single logical Overseer) | 0 (leader election precondition) | `overseer.md §1.5` |
| INV-CH5 (Gateway-persistent Channels survive restart) | 4 (Channel stays `open`) | `channel.md §2.10` |
| INV-CH7 (Recovery re-probes live Channels) | 4 | `schema.md` cross-table |
| INV-GW1 (Gateway transport independence) | 4, 6, 7 | `gateway.md §2.9` |
| INV-B5 (`release_reason` immutable) | 6, 7 (B-T7) | `binding.md §2.3.3` |
| INV-SN1 (Session envelope atomicity, row-lock) | 9, 10 | `session.md §2.7` |
| INV-M4 (`manifests.body` carries augmented shape, served on reconnect) | 5 (SEQ-1 Phase B resume) | `manifest.md §4` |
| INV-P3 (one Event per boundary-crossing transition) | 11 (O-T1/O-T2) | `proto-catalog.md` |
...
| Overseer | `overseer.md §1` (O-T4 Ready -> Draining; -> Stopped) |
| Gateway | `gateway.md §2.9` - closes its RemoteAgent legs as the Overseer drains (IR-O6) |
| Agent (RemoteAgent) | `agent.md §2.1` - **not commanded**; detects closure, enters reconnect loop, keeps running |
| Agent (PlatformAgent) | `agent.md §2.1` - receives Stop Command, drains, terminates (IR-O2) |
| Binding | `binding.md §2.3` - RemoteAgent Bindings -> `draining` (persisted, not released); PlatformAgent Bindings -> released |
| Task | `task.md §2.2` - T-T8 Stopping -> T-T9 Completed (final flush) |
| Channel | `channel.md §2.10` - CH-T3 Draining -> CH-T4 Closed (durable in `channels`) |
| Sink | `sink.md §2.11` - final Parquet flush + close |
...
**Phase A - Drain initiation (O-T4)**

1. **Operator signals drain** - `SIGTERM` to the Overseer process, or an admin drain endpoint. The Overseer fires **O-T4** (Ready -> Draining).
...
3. For each **PlatformAgent** Binding: Overseer composes `Command{AGENT_DRAIN | task_pause->stop, BindingTarget}` and dispatches in-process. Waits up to `service.stop_drain_budget_ms` (2min) for CommandAck. On ack -> T-T8 -> T-T9, CS-T3 -> CS-T4, B-T4 -> B-T6 -> B-T8 (released, `release_reason='normal'`), workspace destroy in the B-T6 tx, A-T6/A-T7 -> drain -> A-T9 terminate. On timeout -> force-stop the process, B-T7 (`release_reason='force'`). (This is structurally SEQ-2 Phase B-F driven by drain rather than delete.)
...
Operator     Overseer            Channels(tbl)     RemoteAgent SDK      Webapp
  |             |                     |                  |                |
  |--SIGTERM--->|--O-T4 Draining----------------------------------------->| health=Draining
...
|---|---|---|
| IR-O2 (Drain asymmetry: RemoteAgent not commanded) | 2, 4, 11 | `overseer.md §1.6` |
| INV-CH3 (Reliability class immutable through drain) | 6, 8 | `channel.md §2.10` |
| INV-CH4 (Sequence monotone within Channel) | 7 (final frame) | `channel.md §2.10` |
| INV-P4 (`restart_epoch` propagates) | 7 | `proto-catalog.md` |
| INV-B4 (Binding monotone progression) | 4 (-> draining) | `binding.md §2.3.3` |
| INV-B-drain-not-release (RemoteAgent Binding stays draining) | 8, 11 | `binding.md §2.3` / IR-O2 |
| INV-SN1 (envelope atomicity) | 3 (PlatformAgent release path) | `session.md §2.7` |
| INV-GW1 (Gateway leg independence) | 9 | `gateway.md §2.9` |
...
- **`draining_reason` column on `bindings`.** Phase C step 4 writes `draining_reason='platform_drain'` to distinguish a platform-drain `draining` Binding (recoverable, SEQ-4 resumes it) from a delete-drain `draining` Binding (SEQ-2, will be released). `binding.md §2.3.1` and `schema.md bindings` should carry a `draining_reason` enum `{operator_stop, platform_drain}` - a one-line schema follow-up; until it lands, the two are disambiguated by whether a `BINDING_RELEASE` Command exists for the Binding (SEQ-2 has one, SEQ-5 does not).
- **`OverseerDraining` / `OverseerStopped` Event variants.** Confirm both exist in `proto-catalog.md events.proto` (O-T4 emits `OverseerDraining`; the Draining -> Stopped transition emits `OverseerStopped`). If only `OverseerDegraded`/`OverseerReady` are wired, add the two drain variants in the typed-Envelope migration (Iteration 5).
- **PlatformAgent docker spawn is the precondition for the Phase B half being live.** Until the PlatformAgent spawn path lands (Iteration 5 backlog), SEQ-5 is exercised RemoteAgent-only; Phase B is spec-complete but untested against a real PlatformAgent.
...
- [fsm-v0.1-beta-2/fsm-main.md - §3 sequences index](fsm-main.md)
- [fsm-v0.1-beta-2/overseer.md - §1.3 Overseer states, §1.4 Matrix, §1.6 IR-O4, §1.7 Recovery](overseer.md)
...
- [fsm-v0.1-beta-2/schema.md - persistence writes named in SEQ-1](schema.md)
- [fsm-v0.1-beta-2/proto-catalog.md - Envelope, Registration, Manifest, Command, events](proto-catalog.md)
- [fsm-v0.1-beta-2/errors.md - failure-branch error kinds](errors.md)
- [fsm-v0.1-beta-2/timeouts.md - Phase timing budgets](timeouts.md)
