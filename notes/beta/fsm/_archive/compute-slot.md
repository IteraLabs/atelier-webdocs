...
│        │          │                                        │                 │
│        │          └───────────────────────────────────────►│                 │
│        │                  (CS-T3 abort before Occupied)    │                 │
│        │                                                   │                 │
│        └──────────────► Retired ◄──────────────────────────┘                 │
│                                                                              │
│  Retired: absorbing terminal for this row (Pipeline topology retirement).    │
│  The Vacant ◄─► Retired edge is Update-driven and deferred in v0.1.          │
└──────────────────────────────────────────────────────────────────────────────┘
```
...
Entry: Pipeline creation (initial write); CS-T3 (abort); CS-T5 (release).
...
Entry: CS-T1.
Exit: CS-T2 (hard reserve by B-T1) or CS-T3 (abort before Occupied).

**S-CS3: Occupied.** Slot is actively hosting a Task. `current_activation_id`, `current_task_id`, `current_binding_id` are all non-NULL. Schema compatibility between Task I/O schemas and Slot I/O schemas was validated at CS-T2 entry; the Task is running (T-T3 fired in the Task FSM) or eligible to run (T-T2 Accepted).

Entry: CS-T2.
Exit: CS-T4 (Task reaches a terminal state, or Binding is released/drained).

**S-CS4: Releasing.** Task has reached a terminal state (Completed, Failed, Canceled) or its Binding is terminating (B-T4, B-T7), and the Slot is winding down its reference: any in-flight Artifact emission from this slot is allowed to drain to ArtifactChannel; the `current_*` fields are preserved for the duration so that lineage queries during the window resolve.

Entry: CS-T4.
Exit: CS-T5 (release complete - slot returns to Vacant).
...
Entry: CS-T6 (deferred), CS-T7 (deferred).
Exit: none (terminal).
...
CS-T2:  Reserved ──► Occupied
        ...
        Effects:  Persist ComputeSlot row: state=occupied, current_task_id,
                  ...
                  (Task FSM fires T-T2 Accepted; Binding FSM fires B-T1 Pending).

CS-T3:  Reserved ──► Vacant
        Trigger:  SV-T4 (Service abort before Dispatch) or SV-T5 (manifest rejected
                  before B-T3 Active) for the activation holding this Slot.
        Guard:    Slot is in Reserved; activation is transitioning to Stopped.
        Effects:  Persist ComputeSlot row: state=vacant, current_activation_id=NULL,
                    reserved_at=NULL.

CS-T4:  Occupied ──► Releasing
        Trigger:  Task T-T* terminal (T-T7 Completed, T-T8 Failed, T-T9 Canceled -
                  reference Task FSM §2.2);
...
CS-T5:  Releasing ──► Vacant
        ...
        Guard:    Slot is in Releasing;
                  for Task T-T7 Completed: ArtifactChannel drained to EOS marker;
                  for T-T8 Failed / T-T9 Canceled: lineage records finalized;
...
CS-T6:  Vacant ──► Retired
        Trigger:  Pipeline topology Update retires this slot_ordinal.
        Guard:    Slot is in Vacant; no Service currently holds
                    current_activation_id.
        Effects:  DEFERRED in v0.1-beta-2 (Update is not in beta scope).

CS-T7:  Occupied ──► Retired
        Trigger:  Forcible retire (not supported in v0.1-beta-2).
        Effects:  DEFERRED. No forcible-retire path from Occupied in v0.1; Update
                    must drain the slot through the normal CS-T4 -> CS-T5 path
                    before retiring.
```

### 2.8.3 Invariants

**INV-CS1: 1:1 Task-Slot runtime relationship.** At any instant, for any ComputeSlot row in state Occupied: exactly one Task (by `current_task_id`) is associated; and for any Task in state Accepted, Running, Completing, Completed, Failed, Canceled: exactly one Slot (by `current_task_id` reverse lookup) is associated. Enforced at CS-T2 boundary (set current_task_id) and CS-T5 boundary (clear current_task_id).
...
**INV-CS6: Release completeness gates Service termination.** SV-T9, SV-T10, SV-T5, SV-T4 require all Slots held by the Service's activation to be in Vacant. Specifically: SV-T5 requires all this activation's Slots in Vacant via CS-T3 (Reserved) or CS-T5 (if any reached Occupied before rejection - which is the spawn-then-reject case); SV-T9/SV-T10 require all Slots in Vacant via CS-T5.
...
- **IR-CSS1:** SV-T1 fires CS-T1 on every Slot in the target Pipeline's topology, atomically (all-or-nothing - if any Slot is not Vacant, SV-T1 is rejected with `PIPELINE_BUSY`).
- **IR-CSS2:** SV-T2 fires CS-T2 on every reserved Slot (exactly once each, via the single Binding's B-T1 in v0.1).
- **IR-CSS3:** SV-T4 fires CS-T3 on every Reserved Slot; SV-T5 fires CS-T3 on Reserved Slots and CS-T4->CS-T5 on any Slots that reached Occupied before the rejection (rare: reject-after-occupy).
- **IR-CSS4:** SV-T6 (Stopping) fires CS-T4 on every Occupied Slot via the B-T4 cascade; the Service then waits in Stopping until every Slot has reached Vacant (INV-CS6).
...
- **IR-CST2:** T-T7 (Task Completed), T-T8 (Failed), T-T9 (Canceled) each fire CS-T4 on the owning Slot.

**ComputeSlot <-> Binding (IR-CSB)**

- **IR-CSB1:** B-T1 effects include CS-T2 firing for every Slot covered by the Manifest.
- **IR-CSB2:** B-T4 (Active -> Draining), B-T6, B-T7, B-T10 fire CS-T4 on every Occupied Slot held by this Binding.
- **IR-CSB3:** B-T8 (Releasing -> Released) requires every Slot covered by the Binding to have reached Vacant (i.e., CS-T5 fired). This is the Binding-level mirror of INV-CS6.

### 2.8.5 Notes

- **Pipeline-scoped rows, single-active-Service constraint.** In v0.1-beta-2, a Pipeline can have at most one active Service/Experiment at a time (INV-CS3). This simplification keeps ComputeSlot state single-valued without introducing per-activation instance rows. A post-beta version may relax to concurrent activations by instancing rows per activation or by extending state to a map.
- **Update-driven transitions deferred.** CS-T6 and CS-T7 are the Update-path transitions; they are enumerated for completeness and deferred with the Update System Operation.
- **Schema subsumption.** INV-CS2's "equal or subsumed by" allows a Task with a more general output schema to occupy a slot with a more specific output schema (covariant output) and a Task with a more specific input schema to occupy a slot with a more general input schema (contravariant input). The precise subsumption rule is part of the Schema subsystem (out of scope for v0.1 FSM work; treat as equality for the beta implementation and tighten in a subsequent ADR).
...
- **Testability.** Every `INV-CS*` is asserted at its transition boundary. `INV-CS1` is the load-bearing invariant and is asserted at both CS-T2 (setting) and CS-T5 (clearing) - violations are structural bugs in the Task<->Slot join logic.
