...
Semantic dispatch guide for the `ErrorKind` enum declared in `proto-catalog.md §common.proto`. Every error kind raised on the wire or on the internal bus is listed here with:
...
**Scope: `ErrorKind` vs. `TaskRejectionReason`.** This catalog enumerates `ErrorKind` only - the wire-level, cross-cutting error enum declared in `proto-catalog.md §common.proto` and carried by the `Error` message on every boundary (`CommandAck.rejected`, `ManifestAck.rejected`, `RegistrationResponse.rejected`, and the `Error` variant of the Envelope `payload` oneof). Distinct from `ErrorKind` is **`TaskRejectionReason`** (`proto-catalog.md §control.proto`, declared adjacent to `ManifestAck`) - a narrow, Task-FSM-local catalog naming *why a single Task was rejected* during ManifestAck dispatch, with members `TASK_REJECTION_SKILL_MISMATCH`, `TASK_REJECTION_SPEC_INVALID`, `TASK_REJECTION_INTERNAL_ERROR`, and `TASK_REJECTION_PEER_REJECTED` (the last dormant under the v0.1-beta-2 single-Task Manifest restriction per taxonomy §Tasks / beta scope and `binding.md §2.3.2 B-T1`). The first three overlap semantically with `ErrorKind` values of the same base name (`SKILL_MISMATCH = 31`, `SPEC_INVALID = 32`, `INTERNAL = 90`) but live in a separate enum so that the post-beta multi-Task `ManifestAck` shape can carry per-Task rejection reasons without bloating `ErrorKind` with Task-local variants. `TaskRejectionReason` is **not** listed in §E.1-§E.10 below; it is documented at its declaration site in `proto-catalog.md` and cross-referenced from `task.md §2.2.2 T-T2`, `binding.md §2.3.2 B-T10`, and `binding.md §2.3.4 IR-BT2`.
...
- **Raised at**: A-T10 (forced terminate), SN-T6 (`expire_reason = 'plan_revoked'`).
...
- **Raised at**: A-T12 (Registered -> Terminated, reconcile orphaned).
- **Observed by**: SDK.
...
### `AGENT_LOST` (23)
- **Raised by**: A-T5 (Ready/Bound -> Lost), commands dispatched after A-T5.
- **Raised at**: heartbeat-gap detector fires A-T5; later Commands reject.
...
- **Raised by**: B-T1 ManifestAck rejected path.
- **Raised at**: Binding Pending creation fails validation.
- **Observed by**: SDK.
- **Semantics**: Umbrella rejection. Usually specialized into `SKILL_MISMATCH` or `SPEC_INVALID`.
- **Retryable**: `after-mutate`.

### `SKILL_MISMATCH` (31)
- **Raised by**: B-T1 guard - Agent's declared Skills do not cover the Manifest's Task requirements.
...
- **Raised at**: SV-T attempted (absent in v0.1-beta-2; see `service.md §2.5`).
- **Observed by**: SDK.
...
These kinds are the wire representations of the four subsystem loss modes catalogued in `overseer.md §1.4`. They appear in two places:
...
| REGISTRATION_EXPIRED (21) | A-T12 reconcile | after-mutate |
| AGENT_NOT_FOUND (22) | Command resolution | no |
| AGENT_LOST (23) | A-T5 | after-mutate |
...
- [fsm-v0.1-beta-2/proto-catalog.md - §common.proto ErrorKind enum](proto-catalog.md)
- [fsm-v0.1-beta-2/overseer.md - §1.4 Operation Availability Matrix](overseer.md)
- [fsm-v0.1-beta-2/agent.md - §2.1 Agent, incl. §2.1.5](atelier/atelier-notes/fsm-v0.1-beta-2/agent.md)
