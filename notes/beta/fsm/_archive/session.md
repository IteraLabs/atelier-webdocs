...
Part of the **State Machine Atlas v0.1-beta-2**. See `fsm-main.md` for the reading guide, notation, §2.4 FSMs - scope status, and §3 Cross-FSM Sequences. Owns the `INV-SN*` invariant prefix. Cross-references: `overseer.md` (§1, §1.6 IR-O4), `service.md` (§2.5), `agent.md` (§2.1). Participates in **SEQ-1 Deploy** (`sequences.md`) - SN-T2 (Active) is the precondition that admits SV-T1 and A-T1; INV-SN1 is the atomicity mechanism backing the Deploy envelope check.
...
SN-T7:  Expired ──► Closed
        Trigger:  Every child Service under this Session has reached Stopped or
                  Archived, and every child Agent has reached Terminated (Agent
                  A-T14 under the cascade).
...
- **IR-SNO1: IR-O4 atomicity mechanism lives here.** The Overseer's IR-O4 envelope check (§1.6 `overseer.md`) is implemented as an atomic operation against this Session's row per INV-SN1. The Overseer does not maintain its own envelope cache; the Session row is the authoritative counter store.
...
- **IR-SNO3: Overseer drives SN-T6 only for explicit administrative triggers.** Subsystem degradation (Overseer T2/T3/T4) does NOT cascade SN-T6. Sessions survive Overseer Degraded mode; children admission is limited by §1.4 matrix, not by Session state transition.
- **IR-SNO4: Expiry cascade under Degraded.** If SN-T4 fires while Overseer is in Degraded mode, the cascade per IR-SNS2 is queued in durable state (Session status=expired, cascade pending) and drains as subsystems return. `SESSION_ENVELOPE_EXCEEDED` is returned for any new Deploy/Run in the interim.
...
- **IR-SNA2: Expiry cascade.** SN-T4 and SN-T6 cascade to each child Agent by driving the Agent toward Terminated via A-T14 (indirectly - the cascade acts on the Agent's Bindings per IR-SNS2; once the Bindings are Released, the Agent follows its own termination path in §2.1). The Session FSM does not directly drive Agent transitions.
- **IR-SNA3: Counter decrement on Agent termination.** A-T14 decrements `concurrent_agents`. Transactional union with A-T14's persist applies.
