...
Companion to `fsm-main.md` and the per-FSM files (`overseer.md`, `agent.md`, `task.md`, `binding.md`, `service.md`, `session.md`, `compute-slot.md`, `gateway.md`, `channel.md`, `sink.md`). Defines the Postgres tables that hold state-bearing rows for every FSM declared in v0.1-beta-2.

Target: PostgreSQL 15+.
Extensions: `pgcrypto` (for `gen_random_uuid()`).

Twelve tables: `sessions`, `pipelines`, `compute_slots`, `workspaces`, `services`, `agents`, `manifests`, `bindings`, `tasks`, `artifacts`, `commands`, `channels`. Four promotions since the pre-plan's 8: `compute_slots` was added per Step 4 ComputeSlot FSM §2.8 (slot state mutates per activation and cannot live in the `pipelines.topology` JSONB without violating Pipeline-immutability); `artifacts` was promoted in v0.1-beta-2 from the deferred-tables list as lineage-only rows so that per-Service (`SVC-*`) Artifact lineage queries required by `use-case.md §4, §6` are durable without promoting payload content into Postgres; `commands` was promoted in v0.1-beta-2 so that `overseer.md §1.7` reconciliation of stale `pending` Commands is durable across Overseer restart (previously in-memory); `channels` was promoted in Iteration 5 so that Channel identity, reliability class, sequence high-water mark, and `restart_epoch` survive Overseer restart (SEQ-4 Crash Recovery) and so that graceful Drain (SEQ-5) has a durable target to walk Draining -> Closed.
...
CREATE TABLE compute_slots (
    ...
    current_activation_id UUID,                             -- services.service_id (experiments post-beta)
...
CREATE TABLE agents (
    ...
    restart_epoch       INTEGER NOT NULL DEFAULT 0,         -- increments on A-T11 Restarting -> Ready
    ...
    terminated_reason   TEXT CHECK (terminated_reason IN (
                                ...
                                -- (A-T15 agent_lost path since W3.3; A-T18 skill_mismatch);
                                -- back-ported into this SSoT in Iteration 5.1.2.
                                'agent_lost', 'skill_mismatch',
                                -- Iteration 5.1.2 (migration 20260604000002): A-T15
                                -- grace-timer expiry (agent.reconnect_grace_ms) plus the
                                -- A-T9 fallbacks cited in timeouts.md §T.2
                                -- (agent.drain_budget_ms / agent.restart_budget_ms) that
                                -- were written but never admitted by this CHECK.
...
CREATE TABLE manifests (
    ...
    -- canonical gRPC ManifestChannel model (`proto-catalog.md §control.proto`).
    ...
    -- path consumes it for v0.1-beta-2. Retained for the post-beta gRPC
    -- ManifestChannel revival and Iteration 5+ Kafka consumers (boot
    -- reconciler republish, federation) where it regains load-bearing
    -- semantics. See `manifest.md §2.4` and §6.
...
-- in `HandshakeResult.toml_config` (`agent.md §2.1.2`, `manifest.md §2.3`).
-- See `manifest.md` for the full body & delivery contract (INV-M1..INV-M8).
...
CREATE TABLE bindings (
    ...
    -- Aligned to `binding.md §2.3.1` release_reason enum (Atlas precedence).
    ...
    -- Added in v0.1-beta-2 Iteration 4.1:
    ...
    -- Iteration 5.4 (migration 20260604000004): distinguishes a
    -- recoverable platform-drain `draining` Binding (SEQ-5 O-T4, RemoteAgent
...
CREATE TABLE tasks (
    ...
    -- Aligned to `task.md §2.2.1` FSM states (Atlas precedence).
    ...
    --  transition today - a separate minor drift flagged for post-beta.)
    status              TEXT NOT NULL CHECK (status IN (
...
--
-- Agent-emitted wire format:   ArtifactFrame         (proto-catalog.md §data.proto)
-- Overseer-emitted wire format: ManifestArtifactFrame (proto-catalog.md §data.proto)
--
-- **Iteration 4.5.4 - insertion path.** Agent-emitted Artifact rows are
...
-- the lineage block per Iteration 4.5.3. The consumer's INSERT is
...
-- `overseer.md §1.7` reconciliation of stale `pending` Commands across
...
-- keyed by `command_id`.
--
-- Wire shape:  `Command` (`proto-catalog.md §control.proto`) -
...
CREATE TABLE commands (
    ...
    session_id          UUID NOT NULL REFERENCES sessions(session_id) ON DELETE RESTRICT,
    -- CommandKind mirrors `proto-catalog.md §control.proto CommandKind`.
    ...
    --                (set by `overseer.md §1.7` step 2 for pre-crash Commands,
...
-- Reconciliation sweep (`overseer.md §1.7` step 2) scans pending Commands:
...
-- deferred list in Iteration 5 so that Channel identity, reliability
...
--
-- Scope note: `channel.md §2.10` specifies Opening/Open as normative and
-- enumerates Draining/Backpressured/Error/Closed/Failed. Iteration 5
-- promotes Draining, Closed, and Failed to normative (CH-T3, CH-T4,
-- CH-T10) for the Drain/Recovery path; Backpressured and Error remain
-- enumerated-only. The CHECK below admits the full set so a later
...
CREATE TABLE channels (
    ...
    -- the Gateway-control Channel promotion path post-beta; every Channel
    ...
    -- Channel category per taxonomy (HOW §Channels) / `channel.md §2.10`.
    ...
    -- Reliability class per `channel.md §2.10.3`. Immutable after Open
    -- (INV-CH3).
    ...
    -- Draining/Closed/Failed promoted to normative in Iteration 5;
...
- **INV-O5** (Reconciliation before Ready) - `overseer.md §1.7` step 2 reads `commands` rows with `status='pending'` and resolves each to `acked` / `rejected` / `timeout` before the Overseer reaches `Ready`. Backed by `commands_pending_issued_idx`.
- **INV-M1** (`[[metadata.tasks]].task_id` in the persisted `manifests.body` is the canonical FK target for `artifacts.task_id`) - owned by `manifest.md`; enforced agent-side at the UpstreamSink emit boundary in `atelier-sdk/atelier-connect` (canonical `task_id` flows from `HandshakeResult.toml_config` -> `parse_metadata_tasks` -> `UpstreamSinkConfig.task_id` -> `ArtifactLineage.task_id`). The schema-side anchor is `artifacts_task_id_fkey`: agent-emitted ArtifactFrames whose lineage carries any non-canonical UUID are rejected at INSERT.
- **INV-M3** (`accept_task` (T-T1) commits before `manifests.body` is persisted) - owned by `manifest.md`; enforced by `Scheduler::deploy_service` ordering in `atelier-overseer`. The walker fires T-T1 for each `TaskSpec` in the composed Manifest BEFORE inserting the `manifests` row, so `manifests.body` carries the canonical `task_id`(s) in `[[metadata.tasks]]` from the moment of B-T1 commit. The schema-side observable: a SELECT on `manifests.body::jsonb -> 'metadata' -> 'tasks'` after Phase A commit yields a non-empty array whose `task_id` values FK-reach `tasks.task_id`.
- **INV-M4** (`manifests.body` carries the FULL augmented shape) - owned by `manifest.md`; the row is composed-and-augmented atomically inside Phase A's transaction stack and never rewritten at runtime. Reads of `manifests.body` post-B-T1-commit always observe the augmented shape including `[metadata]` + `[[metadata.tasks]]`. This is the body the Gateway returns verbatim in the gRPC handshake.
- **INV-CH3** (Reliability class immutable after Open) - owned by `channel.md §2.10`; the FSM driver never UPDATEs `channels.reliability_class` after the row leaves `opening`. A downgrade is a close + re-open (new `channel_id`).
- **INV-CH5** (Gateway-persistent Channels survive A-T8 Restart) - owned by `channel.md §2.10`; A-T8 increments `channels.restart_epoch` (and `agents.restart_epoch`) WITHOUT transitioning `channels.status` out of `open` or resetting `sequence_high`. The schema-side observable: a Channel row that was `open` before A-T8 is still `open` after, with `restart_epoch` incremented and `sequence_high` non-decreasing. Backed by `channels_live_idx`.
- **INV-CH7** (Recovery re-probes live Channels) - owned by `overseer.md §1.7` / SEQ-4; on boot the Overseer scans `channels` rows with `status IN ('opening','open','draining','backpressured','error')` per `binding_id` it reconciles (Stage 1, IR-CHO4) and resolves each to `open` (transport intact) or `failed` (transport gone, triggers B-T7 force-release). Backed by `channels_live_idx`. A Channel left in `draining` past `agent.reconnect_grace_ms` is force-closed to `failed` (SEQ-5 timeout branch), backed by `channels_draining_idx`.

---

## Migration layout

Each table migration is one forward-only file under `atelier-backend/migrations/`:

```
0001_sessions.sql
0002_pipelines.sql
0003_compute_slots.sql
0004_workspaces.sql
0005_services.sql
0006_services_workspaces_fk.sql     -- late FK workspaces.service_id -> services
0007_agents.sql
0008_manifests.sql
0009_bindings.sql
0010_manifests_bindings_fk.sql      -- late FK manifests.binding_id -> bindings
0011_tasks.sql
0012_artifacts.sql
0013_manifests_artifacts_fk.sql     -- late FK manifests.archived_as_artifact_id -> artifacts
0014_commands.sql                   -- durable Command log, backs overseer.md §1.7 step 2
0015_channels.sql                   -- durable Channel state, backs SEQ-4 recovery + SEQ-5 drain
```

No down-migrations in v0.1 - schema changes ship under a new tagged version.

---

## Deferred tables (post-beta)
...
- **`commands`** - now specified above as table 11. Durable request/ack log. Promoted so that `overseer.md §1.7` step 2 (reconcile Commands with `status='pending'`) is a DB scan rather than a recover-from-memory gap across Overseer restart. In-memory tracking alone could not re-deliver or time out pre-crash Commands.

**Promoted in Iteration 5:**

- **`channels`** - now specified above as table 12. Durable Channel identity + provisioning/drain state. Promoted so that SEQ-4 Crash Recovery can re-probe live Channels by `binding_id` after restart, and SEQ-5 graceful Drain has a durable target to walk Draining -> Closed past the drain grace window. `restart_epoch` and `sequence_high` persistence make `restart_epoch` sufficient as the Artifact lineage discriminator across A-T8 Restart (INV-CH5) without an in-memory reconstruction step. The internal Channel states Draining/Closed/Failed are promoted to normative in `channel.md §2.10` as part of this; Backpressured/Error remain enumerated-only (the CHECK admits them so a later promotion needs no migration).
...
**`row_version` optimistic concurrency.** Present on tables where concurrent Overseer replicas may race (`services`, `bindings`, `tasks`, `agents`, `compute_slots`, `sessions`, `commands`). FSM driver UPDATEs include `WHERE row_version = $N`; 0 rows touched -> retry with fresh read. `commands` needs it because the reconciliation sweep (`overseer.md §1.7` step 2) may race with a live CommandAck arriving for the same `command_id` during the boot window.

**Workspace is service-lifecycle-scoped.** `workspaces.service_id` is NOT NULL; a Workspace is destroyed when its Service reaches Stopped (SV-T4 / SV-T5 effects). Multi-Workspace-per-Service is post-beta.
...
- [fsm-v0.1-beta-2/overseer.md - §1.4 Matrix, §1.6 IR-O4, §1.7 Recovery](overseer.md)
...
- [txy-v0.1-beta-2/txy-main.md - Identifiers, Scopes](../txy-v0.1-beta-2/txy-main.md)
