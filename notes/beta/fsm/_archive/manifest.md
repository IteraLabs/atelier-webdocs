...
Cross-references:
- `proto-catalog.md §control.proto` - wire-level `Manifest`, `TaskSpec`, `ManifestAck`, `AssignedTask` messages
- `schema.md` table 7 (`manifests`) - persistence of the augmented body
- `sequences.md §3.1.1` - SEQ-1 narrative; T-T1 at Phase A
- `binding.md §2.3.2 B-T1` - composition + augmentation guard
- `task.md §2.2.2 T-T1` - canonical `task_id` allocation at Phase A
- `agent.md §2.1.2 A-T1` - agent receipt at gRPC handshake
- `overseer.md §1.7` - boot reconciliation (server-internal)
...
1. **Composition + augmentation** - operator-submitted TOML -> Overseer-composed body with platform-allocated IDs injected into a `[metadata]` block, including a `[[metadata.tasks]]` sub-array carrying canonical `task_id`(s). Happens during Phase A (`binding.md §2.3.2 B-T1` + `task.md §2.2.2 T-T1`).
2. **Delivery** - the persisted `manifests.body` is returned verbatim to the agent in the gRPC handshake response (`gateway.md`, `agent.md §2.1.2`, `proto-catalog.md §services.proto`).
...
A server-internal Kafka publish path on topic `control.manifests` exists (`control_publisher::publish_manifest`) and is preserved for future iterations (boot-reconciler republish, multi-Overseer-replica coordination, post-beta Gateway federation). For v0.1-beta-2 the publish is fire-and-forget and is **not load-bearing** - no agent or Gateway consumer depends on it. See §6.
...
**Spec deviation from `sequences.md §3.1.1` step 23**: the spec narrative places T-T1 after `ManifestAck` (step 22) under the canonical gRPC `ManifestChannel` model. Under v0.1-beta-2 BYO-infra T-T1 fires during Phase A, so the body persisted at B-T1 already carries the canonical task_id. See `sequences.md §3.1.1` BYO-infra note and §3.1.6 recovery semantics. INV-M3 (§4) codifies the deviation.
...
3. Opens a gRPC connection to the Gateway and calls `handshake()` per `proto-catalog.md §services.proto::Gateway.Handshake`.
...
### 2.4 Server-internal Kafka publish (Iteration 5+, non-load-bearing in v0.1-beta-2)
...
If `publish_manifest` fails at the broker layer, Phase A still succeeds - the body is persisted, the handshake will return it. The publish error is logged but not fatal. Iteration 5+ work that depends on the topic (boot-reconciler republish, etc.) will need to revisit this trade-off.
...
One entry per `TaskSpec` in the composed Manifest. v0.1-beta-2's single-Task Manifest restriction (B-T1 guard, `binding.md §2.3.2`; taxonomy §Tasks) means the array length is always 1. Forward-compatible with post-beta multi-Task.

| Field              | Type      | Source                            | Notes                                                                                                |
|--------------------|-----------|-----------------------------------|------------------------------------------------------------------------------------------------------|
| `manifest_task_id` | string    | Operator-input correlation handle | Opaque to the platform; never FK'd (INV-M2). v0.1-beta-2 placeholder: `"task-0"` until operator-input task IDs land post-beta. |
...
- **No `session_id`** - the agent receives `session_id` via the Envelope header on subsequent gRPC streams (`proto-catalog.md §common.proto`).
...
Agent emitters MUST populate `ArtifactLineage.task_id` (`proto-catalog.md §data.proto`) with the `task_id` value from the active Manifest's `[[metadata.tasks]]` entry whose `manifest_task_id` matches the worker's operator-input task ID. Use of any other UUID - including agent-locally-minted UUIDs - violates the `artifacts_task_id_fkey` constraint.
...
*Testability:* `rg manifest_task_id atelier-backend/migrations/` returns zero `REFERENCES` clauses.
...
This deviates from `sequences.md §3.1.1` step 23 ordering (which places T-T1 after ManifestAck receipt). The deviation is scoped to the BYO-infra delivery path: the persisted body MUST already carry the canonical task_id at the moment the gRPC handshake handler reads it. The post-beta gRPC `ManifestChannel` path restores spec ordering via `ManifestAck.AssignedTask` - see §7.
...
### INV-M6 - `control.manifests` topic is compacted (Iteration 5+ only)

For the server-internal publish path (§2.4 / §6), the topic MUST have `cleanup.policy=compact`. For v0.1-beta-2 this is **informational only** - no agent or Gateway depends on the compacted semantics. Iteration 5+ work that adds a consumer (boot reconciler, federation) will enforce this constraint at infrastructure setup.

*Testability:* Iteration 5+ infrastructure assertion in the cluster-setup script.
...
If `control_publisher::publish_manifest` fails to ack from the broker (network blip, broker outage, etc.) during Phase A, the publish error is logged at WARN but does NOT roll back Phase A - `manifests.body` is persisted regardless. For v0.1-beta-2 this is acceptable because no agent or Gateway consumer depends on the topic. Iteration 5+ work that wires consumers will revisit this trade-off.

### 5.5 Out of scope for v0.1-beta-2

- Manifest re-composition (operator deploys an updated body to an existing Binding) - single-Manifest-per-Binding for v0.1-beta-2 per `sequences.md §3.1.7`.
...
- Server-internal Kafka consumer failure handling - Iteration 5+ when the topic becomes load-bearing.

---

## 6. Server-internal Kafka publication (Iteration 5+ reference)
...
- **Boot-reconciler republish** - `overseer.md §1.7` Step 1 could republish bindings in `pending` whose `manifests.transmitted_at IS NULL` for a Gateway-side cache.
...
- **`schema.md` table 7 (`manifests`)** - inline comment: `body` holds the FULL augmented body per INV-M4; canonical `task_id` is recoverable via the join `manifests -> bindings -> tasks`. `## Cross-table invariants` section lists INV-M1, INV-M3, INV-M4 with `manifest.md` pointers.
- **`sequences.md §3.1.1`** - Phase A narrative includes `accept_task` (T-T1) + `[[metadata.tasks]]` injection. Step 17 BYO-infra note: handshake-delivery, no Kafka. §3.1.4 invariants table references INV-M3 / INV-M4. §3.1.6 recovery section: Phase A atomicity simplifies the crash-windows case.
- **`binding.md §2.3.2 B-T1`** - Effects line: composition includes T-T1 + `[[metadata.tasks]]` injection; persisted body is the full augmented shape (INV-M4).
- **`agent.md §2.1.2 A-T1`** - Effects line: handshake response carries the full augmented body; agent parses `[[metadata.tasks]]` at receipt; INV-M5 worker-spawn gate triggers from handshake completion, not Kafka.
- **`proto-catalog.md §control.proto`** - status note on `Manifest`: v0.1-beta-2 uses `HandshakeResult.toml_config` for delivery; the gRPC `Manifest` message is reserved for the post-beta gRPC `ManifestChannel` path.
- **`overseer.md §1.7`** - boot reconciler bullet: BYO-infra Manifest republish to Kafka is server-internal (Iteration 5+); v0.1-beta-2 reconciler preserves existing semantics.
- **`INDEX.md`** - entry for `manifest.md` description: "Manifest body & delivery contract (`[metadata]` + `[[metadata.tasks]]`, gRPC handshake delivery)".

---

## 9. Sources

- `fsm-v0.1-beta-2/proto-catalog.md` - `Manifest`, `TaskSpec`, `ManifestAck`, `AssignedTask`, `HandshakeResult` wire messages
- `fsm-v0.1-beta-2/schema.md` table 7 - `manifests` persistence
- `fsm-v0.1-beta-2/sequences.md §3.1` - SEQ-1 Deploy narrative
- `fsm-v0.1-beta-2/binding.md §2.3.2` - B-T1 composition + augmentation guard
- `fsm-v0.1-beta-2/task.md §2.2.2` - T-T1 canonical `task_id` allocation (Phase A under BYO-infra)
- `fsm-v0.1-beta-2/agent.md §2.1.2` - A-T1 registration + Manifest receipt
- `fsm-v0.1-beta-2/overseer.md §1.7` - boot reconciliation
- `txy-v0.1-beta-2/use-case.md §"BYO Market Data Collection"` - operator-side flow context
