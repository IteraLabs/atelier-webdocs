...
Companion document to the **Taxonomy v0.1-beta-2** (see `../txy-v0.1-beta-2/txy-main.md`). The taxonomy defines _what_ each entity is; this atlas defines _how_ each entity behaves over time - its states, transitions, guards, effects, and interactions with other state machines.
...
| File | Contents |
|---|---|
| `fsm-main.md` (this file) | Reading guide, notation, invariant testability convention, §2.4 FSMs scope status, §2.4.1 FSM Ownership Matrix, §3 Cross-FSM Sequences index |
| `overseer.md` | §1 Overseer FSM - dependencies, states, transitions, Operation Availability Matrix, invariants, interaction rules, reconciliation protocol |
| `agent.md` | §2.1 Agent FSM (incl. §2.1.5 RemoteAgent restart lineage resolution) |
| `task.md` | §2.2 Task FSM |
| `binding.md` | §2.3 Binding FSM |
| `service.md` | §2.5 Service FSM |
| `session.md` | §2.7 Session FSM |
| `compute-slot.md` | §2.8 ComputeSlot FSM |
| `gateway.md` | §2.9 Gateway FSM (minimal) |
| `channel.md` | §2.10 Channel provisioning contract (minimal) |
| `sink.md` | §2.11 Sink provisioning contract (minimal) |
| `schema.md` | Postgres persistence schema (DDL for ten tables, incl. lineage-only `artifacts` promoted in v0.1-beta-2) |
| `proto-catalog.md` | Wire-format SSoT: Envelope, control, telemetry, data, events, services |
| `errors.md` | Semantic catalog for the `ErrorKind` enum |
| `timeouts.md` | Catalog of time-bounded decision points and tunables |
| `sequences.md` | Cross-FSM interaction sequences (SEQ-1 Deploy, SEQ-2 Delete, SEQ-4 Crash Recovery, SEQ-5 Platform Drain specified; SEQ-3 folded into SEQ-4) |
...
| FSM | File | Depth |
|---|---|---|
| Overseer | `overseer.md` §1 | full |
| Agent | `agent.md` §2.1 | full (incl. §2.1.5 RemoteAgent restart lineage; A-T16 Ready -> Terminated on AGENT_TERMINATE added for SEQ-2) |
| Task | `task.md` §2.2 | full |
| Binding | `binding.md` §2.3 | full |
| Service | `service.md` §2.5 | full |
| Session | `session.md` §2.7 | full |
| ComputeSlot | `compute-slot.md` §2.8 | full |
| Gateway | `gateway.md` §2.9 | minimal (enough to back INV-GW1 / §2.1.5) |
| Channel | `channel.md` §2.10 | provisioning contract only |
| Sink | `sink.md` §2.11 | provisioning contract only |

**Deferred to post-beta**

- **2.6 Experiment FSM** - States: Provisioning, Running, Producing, Completing, Completed, Failed, Archived. Same shape as Service but with bounded lifecycle. Deferred because the beta scope cut is "Service only". No file yet.
- **Channel full lifecycle (§2.10)** - Draining, Closed, Failed are specified as of Iteration 5 (for SEQ-4/SEQ-5) with durable rows in `schema.md` table 12 (`channels`). Backpressured and Error remain enumerated for cross-reference, not fully specified.
- **Sink full lifecycle (§2.11)** - states Streaming, Writing, Backpressured, Error, Closed are enumerated in `sink.md §2.11` for cross-reference but not fully specified.

---

### 2.4.1 FSM Ownership Matrix

Companion to §2.4. §2.4 declares *which* FSMs exist and at what depth; §2.4.1 declares *who owns* each FSM - which repo executes transitions, persists authoritative state, emits Events, and enforces invariants. Numbered §2.4.1 (not §2.5) because §2.5 is Service FSM (`service.md`) under the Atlas's stable cross-file section IDs - this is the §2.4.x pattern the root `CLAUDE.md` house convention names when §2.4 is taken. Subordinate to the `INDEX.md` Precedence rules: Atlas wins on dynamics, Taxonomy wins on static definition; this section clarifies the reach of "wins on dynamics" across repos.
...
2. Persists authoritative state (a row in the table named by `schema.md`, or authoritative in-memory state backed by persistence).
3. Emits the corresponding `Event` variants on the wire per **INV-P3** (`proto-catalog.md`).
...
An **observer** is a process that subscribes to Event Broadcast (`proto-catalog.md §services.proto` `Gateway.EventSubscribe`) and renders state. Observers never own FSMs; they never persist or emit Events.
...
| FSM | § | Ownership | Owner repo(s) - process | Persistence (per `schema.md`) |
|---|---|---|---|---|
| **Overseer** | §1 | sole | `atelier-backend/atelier-overseer` | in-memory + §1.7 reconciliation over `{sessions, services, bindings, tasks, agents, commands}` |
| **Session** | §2.7 | sole | `atelier-backend/atelier-overseer` | `sessions` |
| **Service** | §2.5 | sole | `atelier-backend/atelier-overseer` | `services` |
| **Binding** | §2.3 | sole | `atelier-backend/atelier-overseer` | `bindings` (IR-O3: Overseer is sole creator/destroyer) |
| **ComputeSlot** | §2.8 | sole | `atelier-backend/atelier-overseer` | `compute_slots` |
| **Gateway** | §2.9 | sole | `atelier-backend/atelier-gateway` | in-memory (stateless by design; see `overseer.md §1.1.1`) |
| **Agent** | §2.1 | co-by-AgentType | RemoteAgent -> `atelier-sdk`; PlatformAgent -> `atelier-backend/atelier-overseer` | `agents` (platform-authoritative mirror of both) |
| **Task** | §2.2 | co-by-AgentType | RemoteAgent tasks -> `atelier-sdk`; PlatformAgent tasks -> `atelier-backend/atelier-overseer` | `tasks` (platform-authoritative mirror of both) |
| **Channel** | §2.10 | co-by-seam | platform end -> `atelier-backend/atelier-gateway`; Agent end -> `atelier-sdk` (RemoteAgent) or `atelier-backend/atelier-overseer` (PlatformAgent) | `channels` durable as of Iteration 5 (`schema.md` table 12); Overseer owns row state, transport co-located at each seam |
| **Sink** | §2.11 | co-by-SinkType | ObjectSink -> producer-local (sdk or backend); DBSink -> `atelier-backend`; TerminalSink -> `atelier-backend` for SK-T1/SK-T2 handshake, `atelier-webapp` for rendering | deferred; `workspaces.sinks` JSONB authoritative in v0.1-beta-2 |
...
- **Agent FSM (co-by-AgentType).** The local Agent process is authoritative between heartbeats. The `agents` row is authoritative for platform queries and for `overseer.md §1.7` reconciliation. On rebinding after A-T14 `Lost -> {Ready, Bound, Draining, Restarting}`, the Overseer reconciles its `agents.status` against the reconnecting Agent's self-reported phase, using `restart_epoch` per §2.1.5 to disambiguate lineage.
...
- Subscribe via `Gateway.EventSubscribe { session_id, transition_ids[] }` (`proto-catalog.md §services.proto`).
- Receive `Envelope { payload: Event { body: ... } }` streams, Session-scoped per **INV-P6** (`proto-catalog.md §Invariants`).
...
#### Interaction with `INDEX.md` precedence
...
- `fsm-main.md §2.4` FSMs scope status
- `overseer.md §1.1.1` Composition (logical vs physical); §1.4 Operation Availability Matrix; key decision #2; IR-O3; IR-O6
- `agent.md §2.1.2 A-T1` (Gateway-allocated AgentId); §2.1.5 RemoteAgent restart lineage; A-T8..A-T14; IR-AT1; IR-AT2
- `binding.md §2.3.2 B-T1` (Overseer-allocated BindingId)
- `task.md §2.2` T-T12 restart participation
- `proto-catalog.md §services.proto` `Gateway.EventSubscribe`; §Invariants INV-P3, INV-P6
- `INDEX.md` Precedence
- `schema.md` table 6 `agents`, table 8 `bindings`, table 9 `tasks`, table 11 `commands`, table 12 `channels` (Iteration 5); deferred `sinks`
...
**SEQ-1: Deploy** - specified in `sequences.md §3.1`. Overseer + Session + Service + Binding + (Agent + Task + ComputeSlot after connection). Closes use-case steps 1-5.
**SEQ-2: Delete** - specified in `sequences.md §3.2`. Overseer + Agent + Task + Binding + ComputeSlot + Service (+ Channel / Sink teardown as transport-layer cleanup). Closes use-case step 6. Introduces `agent.md` A-T16 (Ready -> Terminated on AGENT_TERMINATE).
**SEQ-3: Agent Disconnect (unexpected)** - folded into SEQ-4 Stage 2 (`sequences.md §3.4.6`); the live-ticker analog of heartbeat-miss -> A-T11 Lost -> grace -> A-T14/A-T15. Not given its own section.
**SEQ-4: Overseer Crash Recovery** - specified (Iteration 5, `sequences.md §3.4`). Backed by the durable `channels` table (`schema.md` table 12) so INV-CH5/INV-CH7 survive restart; the cross-FSM choreography of `overseer.md §1.7`.
**SEQ-5: Graceful Shutdown (Platform Drain)** - specified (Iteration 5, `sequences.md §3.5`). Uses SEQ-4's durable Channel state plus the named `agent.reconnect_grace_ms` tunable. Closes use-case step 7; composes with SEQ-4 for the researcher-side resume.
